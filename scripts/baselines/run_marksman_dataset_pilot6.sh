...
OUT=${ROOT}/paper_results/external_baselines/marksman_dataset_pilot
LOG=${ROOT}/logs_external_baselines/marksman_dataset_pilot
STATUS=${OUT}/marksman_dataset_pilot6_status.csv
...
run_one () {
  ...
  if [ "$MODEL" = "resnet18" ]; then
    if [ "$DATASET" = "cifar10" ] || [ "$DATASET" = "gtsrb" ]; then
  ...
  TAG="marksman_${DATASET}_${MODEL}_pr005_seed${SEED}_target${TARGET}"
  RUN_OUT="${OUT}/${TAG}"
...
run_one cifar10  resnet18 0.05 0 0
run_one cifar10  vgg11    0.05 0 0
